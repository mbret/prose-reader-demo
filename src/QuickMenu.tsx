...
export const QuickMenu = ({ open, isComics, onSettingsClick }: {
  ...
  const [absoluteBeginPageIndex = 0, absoluteEndPageIndex = 0] = [pagination?.beginAbsolutePageIndex, pagination?.endAbsolutePageIndex].sort()
            ...
            <div style={{
              ...
              {!isComic && !hasOnlyOnePage && (
                <div style={{
                  ...
                  {!beginAndEndAreDifferent && (
                    <>{`page ${pageIndex} of ${pagination?.beginNumberOfPagesInChapter}`}</>
                  )}
                </div>
              )}
              {isComic && !hasOnlyOnePage && (
