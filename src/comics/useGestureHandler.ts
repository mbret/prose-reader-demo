...
import { isMenuOpenState, readerSettingsState } from "../state";

export const useGestureHandler = (
  container: HTMLElement | undefined,
  isUsingFreeMode: boolean = false
) => {
  const reader = useReader();
  const setMenuOpenState = useSetRecoilState(isMenuOpenState);
  const movingHasStarted = useRef(false);
  const [hammerManager, setHammerManager] = useState<HammerManager | undefined>(
    undefined
  );
  const readerSettings = useRecoilValue(readerSettingsState);
  const isScrollableBook =
    readerSettings?.computedPageTurnMode === `scrollable`;

  const onSingleTap = useCallback(
    ({ srcEvent, target, center }: HammerInput) => {
      if (!reader) return;

      console.log(`SINGLE TAP`, {srcEvent})
  ...
  useEffect(() => {
    ...
    function onPanMove(ev: HammerInput) {
      ...
      const deltaX =
        normalizedEvent && `x` in normalizedEvent
          ? normalizedEvent?.x - movingStartOffsets.x
          : ev.deltaX;
      const deltaY =
        normalizedEvent && `y` in normalizedEvent
          ? normalizedEvent?.y - movingStartOffsets.y
          : ev.deltaY;
